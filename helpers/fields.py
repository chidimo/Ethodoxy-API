...
class AutoSlugField(models.SlugField):
    ...
    def __init__(self, *args, **kwargs):
        ...
        set_using = kwargs.pop('set_using', "blank")
        ...
        if set_using is None:
            Warning(
                "Chosen 'set_using' field is blank. It will be set to \
                'blank' since an empty slug could lead to url reversal errors")
    ...
    def pre_save(self, model_instance, add):
        ...
        if self.set_once:
            if getattr(model_instance, self.attname):# and (): #slug field already has a value
                pass
            else:
...
class ProcessingCharField(models.CharField):
    """
    A CharField that applies a passed function on its value.
    """
    pass